...
def simulated_annealing(init_function, cost_function, random_neighbour, acceptance_probability, temperature, maxsteps=1000,
                        debug=True):
    """Алгоритм відпалу
    ...
    for step in range(maxsteps):
        ...
        if debug and step % 100 == 0:
            print("Step #{:>2}/{:>2} : T = {:>4.3g}, state = {}, cost = {}".format(step, maxsteps, T, state, cost))

        rand = random.random()
        accept = acceptance_probability(cost, new_cost, T)
        if accept > rand:
